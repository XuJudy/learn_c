...
int main(){
    ...
    while(scanf("%d",&n)!=EOF){
        ...
        for(int i=1; i<=n; i++){
            ...
            scanf("%s %d%d %c %c %d", stu[i].name, &stu[i].ave, &stu[i].eva, &stu[i].lead, &stu[i].west, &stu[i].pos);

            if(stu[i].ave>80 && stu[i].pos>=1 && stu[i].pos<=10){

                stu[i].sum = stu[i].sum + p1;
                all = all + p1;
            }
            if(stu[i].ave>85 && stu[i].eva>80){
